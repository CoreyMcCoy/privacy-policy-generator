...
function buildPrompt(answers) {
  ...
  const prompt = `You are an expert privacy attorney tasked with creating a comprehensive, legally-compliant privacy policy. Write a professional privacy policy that is clear, thorough, and follows current privacy law best practices.
...
export async function generatePrivacyPolicy(answers) {
  try {
    ...
    const message = await anthropic.messages.create({
      ...
      max_tokens: 4000,
      temperature: 0.3, // Uses a lower temperature for more consistent legal content
      messages: [
        {
          role: 'user',
          content: prompt,
    ...
    const generatedPolicy = message.content[0].text;
